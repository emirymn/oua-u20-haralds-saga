...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(GetComponentInChildren<Camera>().transform.position, GetComponentInChildren<Camera>().transform.TransformDirection(Vector3.forward), out hit, 5f))
        {
            if (!hit.transform.gameObject.CompareTag("Fish") && !hit.transform.gameObject.CompareTag("Animal") && !hit.transform.gameObject.CompareTag("Wood")
            && !hit.transform.gameObject.CompareTag("NPC"))
            {
                UIManager.instance.infoTextBottom.gameObject.SetActive(false);
                UIManager.instance.infoText.gameObject.SetActive(false);
                Debug.DrawRay(GetComponentInChildren<Camera>().transform.position, GetComponentInChildren<Camera>().transform.TransformDirection(Vector3.forward) * 15f, Color.white);
            }
            ...
            if (hit.transform.gameObject.CompareTag("NPC"))
            {
                ...
                UIManager.instance.infoText.text = "press 'R' to talk to " + hit.transform.gameObject.name;
                Debug.DrawRay(GetComponentInChildren<Camera>().transform.position, GetComponentInChildren<Camera>().transform.TransformDirection(Vector3.forward) * 10f, Color.green);
            }
            else if (hit.transform.gameObject.CompareTag("Wood"))
            {
                Debug.DrawRay(GetComponentInChildren<Camera>().transform.position, GetComponentInChildren<Camera>().transform.TransformDirection(Vector3.forward) * 10f, Color.yellow);
                if (Input.GetMouseButton(0))
                {
                    UIManager.instance.infoTextBottom.gameObject.SetActive(true);
                    UIManager.instance.infoTextBottom.text = "+1 wood";
                }
            }
            else if (hit.transform.gameObject.CompareTag("Animal"))
            {
                Debug.DrawRay(GetComponentInChildren<Camera>().transform.position, GetComponentInChildren<Camera>().transform.TransformDirection(Vector3.forward) * 10f, Color.red);
                if (Input.GetMouseButton(0))
                {
                    UIManager.instance.infoTextBottom.gameObject.SetActive(true);
                    UIManager.instance.infoTextBottom.text = "+1 meat";
                }
            }
            else if (hit.transform.gameObject.CompareTag("Fish"))
            {
                Debug.DrawRay(GetComponentInChildren<Camera>().transform.position, GetComponentInChildren<Camera>().transform.TransformDirection(Vector3.forward) * 10f, Color.blue);
                if (Input.GetMouseButton(0))
                {
                    UIManager.instance.infoTextBottom.gameObject.SetActive(true);
                    UIManager.instance.infoTextBottom.text = "+1 fish";
                }
            }
            else
            {
